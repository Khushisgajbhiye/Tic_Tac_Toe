...
public class Game extends JPanel implements ActionListener {
	...
	public void drawUI(Graphics page) {
		...
		// DRAW score X
		ImageIcon xIcon = new ImageIcon("orangex.jpg");
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (board[i][j] == 0) {
					...
					ImageIcon xIcon = new ImageIcon("orangex.jpg");
					Image xImg = xIcon.getImage();
					page.drawImage(xImg, 5 + offset*i , 5 + offset *j, null);
				} else if (board[i][j] == 2) {
					page.setColor(offwhite);
		...
		frame.addWindowListener(new WindowAdapter() {
			public void windowOpened(WindowEvent e) {
				try {
					...
					game.setPlayerXWins(Integer.parseInt(sc.nextLine()));
					game.setPlayerOWins(Integer.parseInt(sc.nextLine()));
					sc.close();
			...
			public void windowClosing(WindowEvent e) {
				try {
					...
					pw.write("");
					pw.write(game.player1wins + "\n");
					pw.write(game.player2wins + "\n");
					pw.close();
